// Source: https://github.com/pulumi/examples/blob/master/aws-ts-nextjs/nextjs.ts
...
export class NextJsSite extends pulumi.ComponentResource {
  ...
  public constructor(
    ...
    try {
      ...
      execSync("bun install && bunx --yes @opennextjs/aws@3.5.5 build", {
        stdio: "inherit",
        cwd: this.path,
      });
    } catch (error) {
      ...
      const object = new aws.s3.BucketObject(
        `${name}-bucket-object-${hex}`,
        {
          ...
          source: new pulumi.asset.FileAsset(
            path.resolve(this.path, ".open-next/assets", file),
          ),
          cacheControl: file.startsWith("_next/")
            ? cacheControlVersioned
            : cacheControlUnversioned,
    ...
    function makeBehaviour(args: {
      ...
      return {
        allowedMethods: [
          ...
          "HEAD",
          "OPTIONS",
          ...(args.readOnly ? [] : ["PUT", "PATCH", "POST", "DELETE"]),
        ],
    ...
    const queue = new aws.sqs.Queue(`${name}-queue`, {}, { parent: this });

    const serverFunctionRole = new aws.iam.Role(
      `${name}-server-function-role`,
      {
    ...
    const serverFunctionPolicy = new aws.iam.RolePolicy(
      `${name}-server-function-policy`,
      {
    ...
    const serverFunction = new aws.lambda.Function(
      `${name}-server-function`,
      {
        code: new pulumi.asset.FileArchive(
          path.join(this.path, "./.open-next/server-functions/default"),
        ),
    ...
    const imageFunctionRole = new aws.iam.Role(
      `${name}-image-function-role`,
      {
        assumeRolePolicy: aws.iam.assumeRolePolicyForPrincipal({
          Service: "lambda.amazonaws.com",
        }),
    ...
    const imageFunctionPolicy = new aws.iam.RolePolicy(
      `${name}-image-function-policy`,
      {
    ...
    const imageFunction = new aws.lambda.Function(
      `${name}-image-function`,
      {
        code: new pulumi.asset.FileArchive(
          path.join(this.path, "./.open-next/image-optimization-function"),
        ),
    ...
    const serverFunctionUrl = new aws.lambda.FunctionUrl(
      `${name}-server-url`,
      {
    ...
    const serverFunctionInvokePermission = new aws.lambda.Permission(
      `${name}-server-function-invoke-permission`,
      {
    ...
    const imageFunctionUrl = new aws.lambda.FunctionUrl(
      `${name}-image-url`,
      {
    ...
    const imageFunctionInvokePermission = new aws.lambda.Permission(
      `${name}-image-function-invoke-permission`,
      {
        action: "lambda:InvokeFunctionUrl",
    ...
    let hostedZone: pulumi.Output<aws.route53.GetZoneResult> | undefined;
    if (args.customDomain) {
      hostedZone = aws.route53.getZoneOutput({ name: args.customDomain!.name });
    }
    ...
    const distribution = new aws.cloudfront.Distribution(
      `${name}-distribution`,
      {
        aliases: args.customDomain ? [args.customDomain!.name] : [],
        ...
        viewerCertificate: {
          cloudfrontDefaultCertificate: !args.customDomain,
          acmCertificateArn: args.customDomain
            ? "arn:aws:acm:us-east-1:662292074719:certificate/441a3988-039c-482b-aefb-d49473ebeb23"
            : undefined,
          sslSupportMethod: args.customDomain ? "sni-only" : undefined,
        },
        origins: [
          {
    ...
    if (args.customDomain) {
      ...
      new aws.route53.Record("wwwAliasRecord", {
        zoneId: hostedZone!.apply((z) => z.zoneId),
        name: `www.${args.customDomain!.name}`,
        type: "A",
        aliases: [
          {
            name: distribution.domainName,
    ...
    this.domainName = args.customDomain
      ? pulumi.interpolate`${args.customDomain!.name}`
      : distribution.domainName;
    this.url = pulumi.interpolate`https://${this.domainName}`;
  }
}
